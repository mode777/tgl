...
describe("2d.BlendingModes", () => {
    ...
    it('should blend alpha', async() => {
        ...
        const texAlpha = await Texture.fromFile(gl, '../assets/2d/alphatest.png');

        const sprite1 = new Sprite(gl, {
            texture: tex,            
            frame: [128, 128, 128, 128]
        });

        const sprite2 = new Sprite(gl, {
            texture: texAlpha,            
            frame: [0, 0, 128, 128]
        });
        ...
        await expect(gl).toLookLike('./assets/reference/2d-blend-alpha.png', 97)
    })
